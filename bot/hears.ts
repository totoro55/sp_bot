...
const addHears = (bot:Bot<MyContext, MyApi>)=> {
    ...
    bot.hears("Отдел рекламы", async (ctx) => {
        await ctx.reply(`Выберите необходимую операцию`, {reply_markup:
                new Keyboard().text("Отчеты о рекламных конструкциях").row()
    ...
    bot.hears("Отчеты о рекламных конструкциях", async (ctx) => {
        ...
        if (!filial) {
            await ctx.reply(`Не выбран филиал. Для заполнения отчета о рекламных конструкциях необходимо выбрать филиал.`, {reply_markup:new Keyboard()
    ...
    bot.hears("Выбор филиала", async (ctx) => {
        ...
        const filial = ctx.session.selected_filial
        if (!filial) {
            await ctx.reply(`Вы можете осуществить поиск филиала:`, {reply_markup:findFilialKeyboard})
            return
        }
    ...
    bot.hears("Выбрать рекламную конструкцию", async (ctx) => {
        ...
        if (!advStructures || advStructures.length === 0) {
            await ctx.reply(`Не найдены рекламные конструкции по данному филиалу`, {
                reply_markup: new Keyboard()
                    .text("Выбор филиала").row()
                    .text("Назад в меню").row()
            })
    ...
    bot.hears("Завершить добавление файлов", async (ctx) => {
        ...
        if (!report.reportData.photos.length) {
        ...
        if (report.reportId) {
            const res = await updateReport(report, period.id, user)
            if (res.status == "success") {
                await ctx.reply("Отчет успешно обновлен. Вы можете заполнить отчет по другой РК или вернуться на главную.", {
            ...
            if (res.status == "success") {
                await ctx.reply("Отчет успешно добавлен. Вы можете заполнить отчет по другой РК или вернуться на главную.", {
                    reply_markup: new Keyboard().text("Выбрать рекламную конструкцию").row().text("Назад в меню").row()
                })
                ctx.session.marketing_report = null
                return
